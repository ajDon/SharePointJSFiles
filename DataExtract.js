...
var ExtractFromView={
    ...
    CreateModifyLink : function  () {
        var ListGuId=jQuery(".ms-listviewtable").attr("id").split("}")[0]+"}";
        var View=decodeURIComponent(jQuery(".ms-listviewtable").attr("o:webquerysourcehref")).split("View")
    ...
    ExtractButtonEvent : function  () {
        listName = jQuery(".ms-listviewtable").attr("summary");
        var JSON_Columns = [];
        var JSON_LookupAndUser = [];
        jQuery(".ms-listviewtable").find(".ms-vh-div").each(function() {
            var newJSON = [];
            newJSON = {
                "name": jQuery(this).attr("name"),
                "displayname": jQuery(this).attr("displayname"),
                "fieldtype": jQuery(this).attr("fieldtype")
            };
            if (jQuery(this).attr("fieldtype") === "Lookup" || jQuery(this).attr("fieldtype") === "User") {
                JSON_LookupAndUser.push(jQuery(this).attr("name"));
            }
        ...
        if (JSON_Columns.length > 0) {
            jQuery.each(JSON_Columns, function(colIndex, colValue) {
        ...
        jQuery.each(OnlyFeildsWithNullValues,function(i,values){
            CopyOnlyFeildsWithNullValues.push(values.split("/")[0]);
        })
        jQuery.each(Feilds, function(j, f1) {
            ctr = 0;
            filterWithValues = "";
            jQuery.each(FeildValues, function(i, e1) {
                if (e1.indexOf(f1) >= 0) {
                    ctr++;
                    i != 0 & filterWithValues != "" ? filterWithValues += " or " : null;
                    var column=e1.split("/Title")[0];
                    jQuery.inArray(column, CopyOnlyFeildsWithNullValues) >= 0 ? null : filterWithValues += "(" + e1 + ")";
                }
            });
    ...
    FilterJSON : function  (JSON,filterValue,filterColumn) {
        ...
        if(filterColumn.indexOf("/Title")>0){
         ...
         results=jQuery(JSON).filter(function(index,value){
         ...
         var temp=jQuery(JSON).filter(function(index,value){
    ...
    GetFilteredValues : function  (JSON_LookupAndUser) {
            ...
            jQuery.each(filterValues.split("="), function(index, value) {
                if (value.indexOf("FilterValue") >= 0) {
                    if (value.indexOf("-") >= 0) {
                        ColumnName = value.split("-")[0];
                        jQuery.inArray(ColumnName, JSON_LookupAndUser) >= 0 ? ColumnName += "/Title" : null;
                    ...
                    if (value.indexOf("FilterField") >= 0) {
                        ...
                        if (firstValue.indexOf(";#") < 0) {
                            ...
                            jQuery.each(firstValue.split(";#"), function(firstValueIndex, SplitValues) {
                        ...
                        jQuery.each(value.split(";#"), function(newIndex, newValue) {
                ...
                if (jQuery.inArray(ColumnName, OnlyFeildsWithNullValues) >= 0 & ColumnName != "") {
                    var temp = '{"ColumnName" :"' + ColumnName + '","Value" : []}';
                    temp = JSON.parse(temp);
                    if(GetValues[ColumnName].length>0){
                        jQuery.each(GetValues[ColumnName], function(i, ValuesFromGetValues) {
